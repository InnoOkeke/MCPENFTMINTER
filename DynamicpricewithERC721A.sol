...
contract NFT is ERC721A, Ownable, ReentrancyGuard {
  ...
  * @dev Mint `count` tokens if requirements are satisfied.
  */
  function reservedMint(uint256 count) 
  public 
  payable 
  nonReentrant{
    require(owner() == msg.sender || _isPublicMintEnabled, "Mint disabled");
    require(totalSupply() + count <= _reservedSupply, "Exceed max free supply");
    require(count <= _reservedMintLimit, "Cant mint more than mint limit");
    require(count > 0, "Must mint at least 1 token");
  ...
  * Can only be called by the current owner.
  */
  function setReservedSupply(uint256 max) public onlyOwner{
    _reservedSupply = max;
  }
  ...
  * Can only be called by the current owner.
  */
  function setReservedMintLimit(uint256 max) public onlyOwner{
    _reservedMintLimit = max;
  }
  ...
  */
  ...
  function getState() public view returns (bool, uint256, uint256, uint256, uint256, uint256){
    return (_isPublicMintEnabled, _mintCost, _maxSupply, totalSupply(), _reservedSupply, _reservedMintLimit);
  }
  ...
  function getReservedMintLimit() public view returns (uint256) {
    return _reservedMintLimit;
  }
